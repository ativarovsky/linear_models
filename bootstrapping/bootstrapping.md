...
Bootsrapping = repeated sampling but from the same sample (with
replacement).
...
stored in sim\_df\_const. Then I’ll modify this by multiplying the
errors by a term that involves x, and create a new response variable y.
...
– I’d like to get a sense of the variability in estimated intercepts
and slopes across all my bootstrap samples.
...
In comparison to the standard error bands in our previous plot (which
are based on OLS), the distribution of regression lines is narrower near
x=0 and wider at the ends of the x distribution.
...
List columns take some getting used to; there are some materials to help
with that.

  - R for Data Science has a chapter on [fitting many
    models](http://r4ds.had.co.nz/many-models.html)
  - Jenny Bryan’s purrr
    [tutorial](https://jennybc.github.io/purrr-tutorial/) has useful
    list-column examples

Boostrapping and resampling are also new concepts; the materials below
explore these using tidyverse approaches.

  - The modelr [package](https://github.com/tidyverse/modelr) has a page
  - The bootsrapping
    [vignette](https://cran.r-project.org/web/packages/broom/vignettes/bootstrapping.html)
    uses a framework similar to the one we used
